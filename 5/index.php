...
if (!empty($_COOKIE[session_name()]) &&
  ...
  try {
      ...
      if ($user_id) {
          ...
          $stmt = $db->prepare("UPDATE user SET fio = ?, tel = ?, email = ?, gender = ?, bdate = ?, bio = ?, ccheck = ? WHERE id = ?");
          $stmt->execute([
              $_POST['fio'],
              $_POST['tel'],
              $_POST['email'],
              ...
              $_POST['bdate'],
              $_POST['bio'],
              isset($_POST["ccheck"]) ? 1 : 0, // Преобразование boolean в int для БД
              $user_id
          ]);

           // Обновляем user_language (очищаем старые и добавляем новые)
      ...
      print('Error updating data: ' . $e->getMessage());
      exit();
  }
} else {
